...
function Orders() {
  ...
  const set_order = useCallback((order) => dispatch(actions.set_order(order)), [dispatch]);

  const fetch_orders = useCallback(() => dispatch(actions.fetch_orders()), [dispatch]);

  const increment_page = useCallback(() => dispatch(actions.increment_page()), [dispatch]);
  const decrement_page = useCallback(() => dispatch(actions.decrement_page()), [dispatch]);
  ...
  return pages > 0 ? (
    ...
    >
      ...
      <div className="d-flex justify-content-between" style={{ flexBasis: '100%' }}>
        <MdNavigateBefore
          role="button"
          aria-disabled="true"
