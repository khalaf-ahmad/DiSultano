...
function ProductContainer(props) {
  ...
  const fetch_products = useCallback(() => dispatch(actionTypes.fetch_products()), [dispatch]);

  const handle_product_selected = useCallback((product) => set_product({ ...product }), []);

  const reset_product = useCallback(() => set_product(get_initial_product()), []);
  ...
  return (
    <>
      ...
      <Row style={row_height_style}>
        <ProductList products={products} on_product_selected={handle_product_selected} />
      </Row>
      <Row>
        <ProductBuilder
          product={product}
